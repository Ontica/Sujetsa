...
namespace Empiria.Trade.Integration.ETL.Transformers {
  ...
  public class OrderItemsRemTransformer {
    ...
    public FixedList<OrderItemsData> Transform(FixedList<OrderItemsRemNK> toTransformData) {
      ...
      var remls = toTransformData.Select(x => x.Reml).Distinct().ToList();
      ...
      var orderCache = PreloadOrderData(dataServices, remls);
    ...
    private Dictionary<string, OrderCacheData> PreloadOrderData(TransformerDataServices dataServices, List<string> remls) {
      var cache = new Dictionary<string, OrderCacheData>();
      foreach (var reml in remls) {
        cache[reml] = new OrderCacheData {
    ...
    private OrderItemsData Transform(OrderItemsRemNK source,
      ...
      var orderData = orderCache[source.Reml];
      var keywords = EmpiriaString.BuildKeywords(source.Reml, source.Producto, source.Descuento);
      var isNewItem = source.OldBinaryChecksum == 0;

      // JSON constante puede ser pre-serializado
      const string extData = "{\"Name\":\"Remision\"}";
